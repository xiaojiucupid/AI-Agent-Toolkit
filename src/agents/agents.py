...
# 用类型别名统一表示 LangGraph 的不同 Agent 图类型
# - @entrypoint 函数返回 Pregel
# - StateGraph().compile() 返回 CompiledStateGraph
AgentGraph = CompiledStateGraph | Pregel  # get_agent() 返回的类型（始终已加载）
AgentGraphLike = CompiledStateGraph | Pregel | LazyLoadingAgent  # 注册表中可存储的类型
...
def get_agent(agent_id: str) -> AgentGraph:
    ...
    # 如果是延迟加载的 agent，先确保其已加载，再返回对应的图
